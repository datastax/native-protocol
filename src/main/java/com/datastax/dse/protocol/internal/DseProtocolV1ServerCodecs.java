...
public class DseProtocolV1ServerCodecs implements FrameCodec.CodecGroup {
  ...
  public void registerCodecs(Registry registry) {
    registry
        .addDecoder(new AuthResponse.Codec(DSE_V1))
        .addDecoder(new DseBatchCodec(DSE_V1))
        .addDecoder(new DseExecuteCodec(DSE_V1))
        .addDecoder(new Options.Codec(DSE_V1))
        .addDecoder(new DsePrepareCodec(DSE_V1))
        .addDecoder(new DseQueryCodec(DSE_V1))
        .addDecoder(new Register.Codec(DSE_V1))
        .addDecoder(new Startup.Codec(DSE_V1))
        .addDecoder(new Revise.Codec(DSE_V1));

    registry
        .addEncoder(new AuthChallenge.Codec(DSE_V1))
        .addEncoder(new Authenticate.Codec(DSE_V1))
        .addEncoder(new AuthSuccess.Codec(DSE_V1))
        .addEncoder(
            new Error.Codec(
                DSE_V1,
                // OSS C* errors
        ...
        .addEncoder(new Event.Codec(DSE_V1))
        .addEncoder(new Ready.Codec(DSE_V1))
        .addEncoder(
            new Result.Codec(
                DSE_V1,
        ...
        .addEncoder(new Supported.Codec(DSE_V1));
  }
}
